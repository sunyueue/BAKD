...
class Trainer(object):
    def __init__(self, args):
        ...
        self.criterion = SegCrossEntropyLoss(ignore_index=args.ignore_label,reduction='none').to(self.device)
        ...
        for iteration, (images, targets, distances) in enumerate(self.train_loader):
            ...
            with torch.cuda.amp.autocast():
                ...
                if self.args.aux:
                    ...
                    #task_loss_1 = self.CEcriterion(s_outputs[0], targets) + 0.4 * self.criterion(s_outputs[1], targets)
                ...
                exp_variance_scale = torch.exp(-variance_s)
            ...
            scaler.scale(D_losses).backward()
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                ...
                writer.add_scalar('Train/Loss_task', task_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_kd', kd_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_adv_G', adv_G_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_D', D_losses_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_skd', skd_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_cwd_fea', cwd_fea_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_cwd_logit', cwd_logit_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_ifv', ifv_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_at', at_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_fitnet', fitnet_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_psd', psd_loss_reduced.item(), iteration)
                writer.add_scalar('Train/Loss_csd', csd_loss_reduced.item(), iteration)
