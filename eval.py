...
def parse_args():
    ...
    # checkpoint and log
    parser.add_argument('--pretrained', type=str, default='pretrain/BAKD/kd_deeplabv3_resnet18_vaihingen_best_model.pth',
    ...
    def eval(self):
        ...
        for i, (image, target, filename) in enumerate(self.val_loader):
            ...
            color_map = np.array([[255, 255, 255], [255, 0, 0],
                                  ...
                                  [0, 0, 255]])
            image_save = np.zeros((512, 512, 3), dtype=np.uint8)
            image_save = image_save.reshape(-1, 3)
            for ind, pixel in enumerate(result_i):
                image_save[ind] = color_map[pixel]
            image_save = image_save.reshape(512, 512, 3)
            ...
            if self.args.save_pred:
                ...
                for ind, pixel in enumerate(predict):
                    image_save[ind] = color_map[pixel]
                image_save = image_save.reshape(512, 512, 3)
                save_path = os.path.join(output_dir, filename + ".png")
                import cv2
