...
class DeepLabV3_plus(nn.Module):
    ...
    def __init__(self, nclass, backbone='xception', aux=False, local_rank=None, pretrained_base=True, **kwargs):
        super(DeepLabV3_plus, self).__init__(nclass, aux, backbone, local_rank, pretrained_base=pretrained_base, **kwargs)
        self.aux = aux
    ...
    def forward(self, x):
        ...
        x = self.head(c4, c1)
        x_feat_after_aspp = self.head(c4)
...
def get_deeplabv3_plus(backbone='resnet50', local_rank=None, pretrained=None,
                  pretrained_base=True, num_class=19, **kwargs):

    model = DeepLabV3_plus(num_class, backbone=backbone, local_rank=local_rank, pretrained_base=pretrained_base, **kwargs)
    if pretrained != 'None':
