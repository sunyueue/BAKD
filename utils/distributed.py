...
#__init__初始化代码主要是设置了本worker节点的各种信息，比如数据集dataset，rank（全局GPU序号），num_replicas 副本数目。并且计算出来所有样本数目total_size
   #num_replicas ：参与分布式训练的进程数目，如果没有设置，则从group之中得到world_size作为进程数目。rank : 当前进程的序号，如果没有设置，则从group之中得到。
    def __init__(self, dataset, num_replicas=None, rank=None, shuffle=True):
        ...
        self.rank = rank
        self.epoch = 0 #epoch ：每次epoch都会shuffle数据集，如何保持shuffle之后数据集一致性？就是通过epoch完成
...
#DistributedSampler的分配方法是：每段连续的 num_replicas 个数据被拆成一个一个，分给 num_replicas 个进程，而且是通过每个worker 的 rank 来获取数据，这样就达到了不重叠不交叉的目的，但也要注意的是：这样每个进程拿到的数据是不连续的。
        # subsample
